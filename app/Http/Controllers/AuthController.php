...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        ->where(function ($query) use ($request) {
            $query->where('username', $request->username)
                ->orWhere('email', $request->username); // Check both username and email
        })
        ->where('login_type', $request->login_type) // Ensure login type matches
        ->first();
    ...
    {
        $user = session('user');
        $redirectRoute = '/sign-in'; // Default logout redirection
