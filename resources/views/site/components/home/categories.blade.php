		...
		<div class="row justify-content-between align-items-center">
			<div class="col-lg-8">
				<div class="title-three">
					<h2 class="fw-600 text-dark">Most Demanding Categories.</h2>
				</div>
			</div>
			<div class="col-lg-4">
			...
			@foreach($categories as $index => $category)
				<div class="card-style-one text-center mt-20 wow fadeInUp" data-wow-delay="0.{{ $index }}s">
					<a href="{{ url('/jobs/' . $category->cat_href) }}" class="wrapper {{ $loop->first ? 'active' : '' }}">
						...
						<div class="title fw-500">{{ $category->cat_name }}</div>
						<div class="total-job">{{ rand(6, 20) }}k+ Jobs</div> {{-- Or use actual job count if available --}}
					</a>
				</div>
			@endforeach
		</div>		
